...
export default function EditDeliveryPage() {
  ...
  const { allEquipments, updateEquipment, isLoading: isLoadingEquipments } = useEquipments();
  ...
  useEffect(() => {
    if (equipment && !isInitialized) {
      ...
      setClienteIraAvisar(equipment.cliente_ira_avisar);
      setLocation({ lat: equipment.latitude, lng: equipment.longitude });
      setPhoto(equipment.foto_url || equipment.foto_local_path || null);
    ...
    <div className="min-h-screen bg-background pb-safe-area-bottom">
      ...
      <div className="sticky top-0 z-10 glass border-b px-4 py-3 safe-area-top">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-lg font-semibold">Editar Entrega</h1>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="p-4 space-y-4">
        ...
        >
          {isSubmitting ? <LoadingSpinner size="sm" /> : 'Salvar Alterações'}
        </Button>
      </form>
    </div>
  );
